"""
4_integrate.py — LegalLens Phase 3
Patches src/analysis.py to use the fine-tuned LoRA model
instead of zero-shot classification.

Run: python 4_integrate.py
...
BACKUP_PATH   = os.path.join("..", "src", "analysis_v2_backup.py")
MODEL_DIR     = os.path.join("models", "legallens-legal-bert-lora")
...
def main():
    print("LegalLens Phase 3 — Integration")
    print("=" * 50)

    # Check model exists
    if not os.path.exists(MODEL_DIR):
        print(f"❌ Model not found at {MODEL_DIR}")
        print("   Run 2_finetune_lora.py first.")
        return

    # Check analysis.py exists
    if not os.path.exists(ANALYSIS_PATH):
        print(f"❌ src/analysis.py not found at {ANALYSIS_PATH}")
        print("   Make sure you run this from the finetune/ directory.")
        return

    # Backup original
    shutil.copy(ANALYSIS_PATH, BACKUP_PATH)
    print(f"Backed up original analysis.py -> src/analysis_v2_backup.py")

    # Write new analysis.py (utf-8 required for Windows cp1252 compatibility)
    with open(ANALYSIS_PATH, "w", encoding="utf-8") as f:
        f.write(NEW_ANALYSIS)
    print(f"Patched src/analysis.py with fine-tuned model support")

    print("\nIntegration complete!")
    print("   Your app will now automatically use the fine-tuned model.")
    print("   Run from your project root: python -m streamlit run app.py")
    print("\n   To revert: copy src/analysis_v2_backup.py -> src/analysis.py")
